      ...
      <div class="x_content1">
        <form class="form-horizontal form-label-left" >
          ...
          <div class="item form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input type="text" class="form-control col-md-7 col-xs-12" 
              	value="<c:forEach items="${flatFormList}" var="flatFormList"><c:if test="${flatFormList.valueId==appInfo.flatformId }"> ${flatFormList.valueName }</c:if></c:forEach>" readonly="readonly">
            </div>
          </div>
          <div class="item form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input type="text" class="form-control col-md-7 col-xs-12" 
              	value="<c:forEach items="${AppcategoryList }" var="AppcategoryList"><c:if test="${AppcategoryList.id==appInfo.categoryLevel1 }"> ${AppcategoryList.categoryName } -></c:if></c:forEach><c:forEach items="${AppcategoryList }" var="AppcategoryList"><c:if test="${AppcategoryList.id==appInfo.categoryLevel2 }"> ${AppcategoryList.categoryName } -> </c:if></c:forEach><c:forEach items="${AppcategoryList }" var="AppcategoryList"><c:if test="${AppcategoryList.id==appInfo.categoryLevel3 }"> ${AppcategoryList.categoryName }</c:if></c:forEach>" readonly="readonly">
            </div>
          </div>
          <div class="item form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
            	<input type="text" class="form-control col-md-7 col-xs-12" 
              	value="<c:forEach items="${statusList}" var="statusList"><c:if test="${statusList.valueId==appInfo.status }">${statusList.valueName }</c:if></c:forEach>" readonly="readonly">
            </div>
          </div>
          <div class="item form-group">
						...
						<tbody>
							<c:forEach var="appVersion" items="${appVersionList }" varStatus="status">
								<tr role="row" class="odd">
									...
									<td><c:forEach items="${statusList}" var="statusList">
										<c:if test="${statusList.valueId==appVersion.publishStatus }">
										${statusList.valueName }
										</c:if>
									</c:forEach></td>
									<td>
