					...
					<a href="${pageContext.request.contextPath}/dev/flatform/app/appinfoadd" class="btn btn-success btn-sm">新增APP基础信息</a>
							...
							<tbody>
								<c:forEach var="appInfo" items="${appInfoList }" varStatus="status">
									<tr role="row" class="odd">
										...
										<td>
										<c:forEach items="${flatFormList}" var="flatFormList">
											<c:if test="${flatFormList.valueId==appInfo.flatformId }"> 
													${flatFormList.valueName }
											</c:if>
										</c:forEach>
										</td>
										<td>
										<c:forEach items="${AppcategoryList }" var="AppcategoryList">
										<c:if test="${AppcategoryList.id==appInfo.categoryLevel1 }"> 
										${AppcategoryList.categoryName } ->
										</c:if>
										</c:forEach>
										<c:forEach items="${AppcategoryList }" var="AppcategoryList">
										<c:if test="${AppcategoryList.id==appInfo.categoryLevel2 }"> 
										 ${AppcategoryList.categoryName } -> 
										</c:if>
										</c:forEach>
										<c:forEach items="${AppcategoryList }" var="AppcategoryList">
										<c:if test="${AppcategoryList.id==appInfo.categoryLevel3 }">
										  ${AppcategoryList.categoryName }
										</c:if>
										</c:forEach>
										</td>
										<td><span id="appInfoStatus${appInfo.id}">
										<c:forEach items="${statusList}" var="statusList">
										<c:if test="${statusList.valueId==appInfo.status }">
										...
										<c:if test="${appVersionList.id==appInfo.versionId }">
											${appVersionList.versionNo }
										</c:if>
										</c:forEach>		
										</td>
										<td>
                      ...
                      <ul class="dropdown-menu" role="menu">
                        ...
                        <li><a class="modifyVersion" 
											...
											<c:forEach items="${statusList}" var="statusList">
												<c:if test="${statusList.valueId==appInfo.status }">
													statusname="${statusList.valueName}"
												</c:if>
											</c:forEach>									
